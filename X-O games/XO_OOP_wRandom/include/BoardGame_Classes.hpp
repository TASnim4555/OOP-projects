...
class Player {
    ...
    public:
        ...
        virtual void Four_get_move(int& x, int& y);
...
class Four_RandomPlayer: public Player {
protected:
    int dimension;
public:
    // Take a symbol and pass it to parent
    Four_RandomPlayer (char symbol, int dimension);
    // Generate a random move
    void Four_get_move(int& x, int& y);
};
///////////////////////////////////////////
class GameManager {
    private:
        ...
        Player* players[2]{};
    public:
        GameManager(Board* boardptr, Player* playerPtr[2]);
        void run();
    void Four_run();
...
class Pyramid_Tic_Tac:public Board{
public:
    Pyramid_Tic_Tac();//empty constructor
...
class Pyramid_Tic_Tac_RandomPlayer:public RandomPlayer{
private:
    int dimension2;
public:
    // make computer choose randomly from the empty cells
    Pyramid_Tic_Tac_RandomPlayer(char symbol,int dimension,int dimension2);
    void get_move(int &x, int &y) override;
};
class _5x5_X_O_Board:public Board{
public:
    _5x5_X_O_Board();
    bool update_board (int x, int y, char mark);
    void display_board();
